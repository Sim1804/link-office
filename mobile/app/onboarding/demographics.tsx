...
export default function DemographicsScreen(){
 const{token}=useAuth(); const[form,setForm]=useState<Demographics>(initial); const[busy,setBusy]=useState(false); const[loading,setLoading]=useState(true); const[picker,setPicker]=useState<"country"|"department"|null>(null); const[search,setSearch]=useState("");
 ...
 const relationshipDetail=form.relationshipStatus=== "En couple" ? "" : form.relationshipStatus;
 const canSubmit=!!form.gender&&!!form.ageRange&&!!form.country&&!!form.occupation&&!!relationshipDetail&&!!form.livingSituation;
 ...
 <Section n="5" title="Situation sentimentale">{chips(RELATIONSHIPS,form.relationshipStatus,v=>set("relationshipStatus",v))}{form.relationshipStatus==="En couple"&&<View style={{marginTop:10}}>{chips(COUPLE_DETAILS,form.relationshipStatus,v=>set("relationshipStatus",v))}</View>}</Section>
