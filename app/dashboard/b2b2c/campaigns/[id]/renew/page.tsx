...
export default function RenewCampaignPage() {
  ...
  const handleRenew = async () => {
    ...
    try {
      ...
      setSuccess(true);
      setTimeout(() => router.push("/dashboard/b2b2c/campaigns/" + data.campaign.id), 1500);
    } catch (err: any) {
      ...
      <main className="page-main">
        ...
        <div style={{ maxWidth:700, margin:"0 auto", position:"relative", zIndex:1 }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:28 }}>
            <Link href={"/dashboard/b2b2c/campaigns/" + id} style={{ color: "var(--text-3)", display:"flex", alignItems:"center", gap:4, textDecoration:"none", fontSize:13 }}>
          ...
          <div style={{ marginBottom:28 }}>
            <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:8 }}>
              ...
              <h1 style={{ fontFamily:"'Plus Jakarta Sans',Inter,sans-serif", fontWeight:800, fontSize:24, color:"var(--text-1)" }}>Renouveler la campagne</h1>
            </div>
            {parent && (
              <p style={{ color: "var(--text-3)", fontSize:14 }}>
                Renouvellement de : <strong style={{ color: "var(--text-3)" }}>{parent.title}</strong> ({parent.offer})
              </p>
            )}
          </div>
          ...
          {success ? (
            <div className="card" style={{ textAlign:"center", padding:"40px 32px" }}>
              ...
              <h2 style={{ color:"var(--text-1)", fontWeight:700, fontSize:18, marginBottom:8 }}>Campagne renouvelee !</h2>
              <p style={{ color: "var(--text-3)", fontSize:14 }}>Redirection en cours...</p>
            </div>
          ) : (
            <div style={{ display:"flex", flexDirection:"column", gap:20 }}>
              {/* Choix offre */}
              <div className="card">
                <h2 style={{ fontSize:16, fontWeight:700, color:"var(--text-1)", marginBottom:16 }}>Offre pour la nouvelle periode</h2>
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
                  {[
                    { id:"PREMIUM", icon:Zap, color:"var(--primary)", border:"#7c3aed", title:"PREMIUM", sub:"Parcours complet sans module Binome" },
                    { id:"PREMIUM_PLUS", icon:Crown, color:"var(--amber)", border:"#f59e0b", title:"PREMIUM+", sub:"Tout PREMIUM + module Binome Relationnel" },
                  ].map(plan => {
                    const isSelected = form.offer === plan.id;
                    const Icon = plan.icon;
                    return (
                      <div key={plan.id} onClick={() => setForm(f => ({ ...f, offer: plan.id as any }))} style={{ cursor:"pointer", padding:"16px 20px", borderRadius:14, border:"1px solid " + (isSelected ? plan.border+"80" : "var(--border)"), background: isSelected ? plan.color+"12" : "var(--surface-2)", transition:"all 0.2s", outline: isSelected ? "2px solid " + plan.border + "50" : "none", outlineOffset:2 }}>
                        <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:6 }}>
                          <Icon size={18} style={{ color:plan.color }} />
                          <span style={{ fontSize:15, fontWeight:700, color:"var(--text-1)" }}>{plan.title}</span>
                          {isSelected && <span style={{ marginLeft:"auto", width:18, height:18, borderRadius:"50%", background:plan.color, display:"flex", alignItems:"center", justifyContent:"center", fontSize:10, fontWeight:900, color: "white" }}>✓</span>}
                        </div>
                        <p style={{ fontSize:12, color: "var(--text-3)" }}>{plan.sub}</p>
                      </div>
                    );
                  })}
                </div>
                {toPlus && (
                  <div style={{ marginTop:14, padding:"12px 16px", background:"rgba(245,158,11,0.08)", border:"1px solid rgba(245,158,11,0.2)", borderRadius:12, fontSize:13, color:"var(--amber)", display:"flex", alignItems:"flex-start", gap:8 }}>
                    <CheckCircle2 size={14} style={{ marginTop:2, flexShrink:0 }} />
                    Le module Binome Relationnel sera active pour cette nouvelle campagne.
                  </div>
                )}
                {fromPlus && (
                  <div style={{ marginTop:14, padding:"12px 16px", background:"rgba(244,63,94,0.08)", border:"1px solid rgba(244,63,94,0.2)", borderRadius:12, fontSize:13, color:"var(--rose)", display:"flex", alignItems:"flex-start", gap:8 }}>
              ...
              {/* Parametres */}
              <div className="card">
                <h2 style={{ fontSize:16, fontWeight:700, color:"var(--text-1)", marginBottom:20 }}>Parametres de la nouvelle campagne</h2>
                <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
                  ...
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16 }}>
                    <div><label style={{ display:"block", fontSize:12, color: "var(--text-3)", fontWeight:600, marginBottom:6 }}>Date de debut *</label>
                  ...
                  <div><label style={{ display:"block", fontSize:12, color: "var(--text-3)", fontWeight:600, marginBottom:6 }}>Population cible</label>
              ...
              {error && (
                <div style={{ display:"flex", alignItems:"center", gap:8, color:"var(--rose)", fontSize:13, padding:"10px 16px", background:"rgba(244,63,94,0.08)", border:"1px solid rgba(244,63,94,0.2)", borderRadius:10 }}>
