        ...
        <table style={{ width: "100%", borderCollapse: "collapse", textAlign: "left" }}>
          <thead>
            <tr style={{ background: "rgba(30,41,59,0.8)", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
              <th style={{ padding: "12px 16px", color: "#94a3b8", fontWeight: 600, fontSize: 12, textTransform: "uppercase", letterSpacing: "0.05em" }}>Nom & Code</th>
              <th style={{ padding: "12px 16px", color: "#94a3b8", fontWeight: 600, fontSize: 12, textTransform: "uppercase", letterSpacing: "0.05em" }}>Type</th>
              <th style={{ padding: "12px 16px", color: "#94a3b8", fontWeight: 600, fontSize: 12, textTransform: "uppercase", letterSpacing: "0.05em" }}>Contact Admin</th>
              <th style={{ padding: "12px 16px", color: "#94a3b8", fontWeight: 600, fontSize: 12, textTransform: "uppercase", letterSpacing: "0.05em", textAlign: "center" }}>Campagnes</th>
              <th style={{ padding: "12px 16px", color: "#94a3b8", fontWeight: 600, fontSize: 12, textTransform: "uppercase", letterSpacing: "0.05em", textAlign: "center" }}>Utilisateurs</th>
                ...
                <tr key={org.id} style={{ borderBottom: "1px solid rgba(255,255,255,0.03)", transition: "background 0.2s" }} className="table-row-hover">
                  <td style={{ padding: "12px 16px" }}>
                    <Link href={`/dashboard/superadmin/organizations/${org.id}`} style={{ textDecoration: "none" }}>
                      <div style={{ fontWeight: 600, color: "#f8fafc", fontSize: 14, cursor: "pointer" }}>{org.name}</div>
                    </Link>
                    <div style={{ color: "#c084fc", fontSize: 11, marginTop: 4, fontWeight: 700, letterSpacing: "0.05em" }}>{org.codeAccess}</div>
                  </td>
                  <td style={{ padding: "12px 16px" }}>
                    <span style={{ 
                      padding: "2px 8px", 
                  ...
                  <td style={{ padding: "12px 16px" }}>
                    {admin ? (
                      <>
                        <div style={{ color: "#e2e8f0", fontSize: 13 }}>{admin.firstName} {admin.lastName}</div>
                        <div style={{ color: "#94a3b8", fontSize: 12, marginTop: 2 }}>{admin.email}</div>
                      </>
                    ) : (
                      <span style={{ color: "#64748b", fontSize: 12, fontStyle: "italic" }}>Aucun administrateur</span>
                    )}
                  </td>
                  <td style={{ padding: "12px 16px", textAlign: "center", color: "#e2e8f0", fontSize: 14, fontWeight: 600 }}>
                    {org._count.campaigns}
                  </td>
                  <td style={{ padding: "12px 16px", textAlign: "center" }}>
                    <div style={{ display: "inline-flex", alignItems: "center", gap: 6, color: "#94a3b8", fontSize: 13, fontWeight: 600 }}>
                      <Users size={14} /> {org._count.users}
                    </div>
                  </td>
                  <td style={{ padding: "12px 16px", textAlign: "right" }}>
                    <Link href={`/dashboard/superadmin/organizations/${org.id}`} style={{ textDecoration: "none" }}>
                      <Button variant="secondary" size="sm" style={{ padding: "6px 12px" }}>
                        Voir
                      </Button>
                    </Link>
                  </td>
