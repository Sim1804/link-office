...
function Section({ title, icon, children }: { title: string; icon: React.ReactNode; children: React.ReactNode }) {
  return (
    <section style={{ background: "rgba(17,24,39,0.7)", backdropFilter: "blur(12px)", border: "1px solid var(--border)", borderRadius: 16, padding: 28, marginBottom: 20 }}>
    ...
    <>
      ...
      <main style={{ minHeight: "100vh", background: "var(--bg)", paddingTop: 88, paddingBottom: 64, position: "relative" }}>
        ...
        <div style={{ maxWidth: 860, margin: "0 auto", padding: "0 24px", position: "relative", zIndex: 1 }}>
          ...
          {/* Navigation */}
          <button onClick={() => router.push(`/dashboard/b2g/campaigns/${id}`)}
            style={{ display: "flex", alignItems: "center", gap: 8, color: "var(--text-3)", background: "none", border: "none", cursor: "pointer", marginBottom: 24, fontSize: 14 }}>
          ...
          <div style={{ background: "linear-gradient(135deg, rgba(6,182,212,0.08), rgba(124,58,237,0.05))", border: "1px solid rgba(6,182,212,0.15)", borderRadius: 20, padding: 32, marginBottom: 24 }}>
            <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", flexWrap: "wrap", gap: 16 }}>
              <div>
                ...
                <h1 style={{ fontFamily: "'Plus Jakarta Sans', Inter, sans-serif", fontWeight: 800, fontSize: 26, color: "var(--text-1)", marginBottom: 8 }}>
                  {campaign?.title ?? "Campagne"}
                </h1>
                <div style={{ display: "flex", flexWrap: "wrap", gap: 14, fontSize: 13, color: "var(--text-2)" }}>
                ...
                <p style={{ fontSize: 12, color: "var(--text-2)", marginTop: 10 }}>Rapport généré le {generatedAt} • Données anonymisées et agrégées</p>
              </div>
              <button
          ...
          {stats?.anonymityBlocked && (
            <div style={{ background: "rgba(245,158,11,0.08)", border: "1px solid rgba(245,158,11,0.2)", borderRadius: 16, padding: 32, textAlign: "center" }}>
              ...
              <p style={{ color: "var(--text-3)", maxWidth: 460, margin: "0 auto" }}>
                Le rapport ne peut pas être généré avec moins de <strong style={{ color: "var(--text-1)" }}>{stats.threshold} répondants</strong>.
          ...
          {!stats?.anonymityBlocked && stats?.averages && (
            <>
              ...
              <Section title="1. Contexte et méthodologie" icon={<FileText size={16} style={{ color: "#06b6d4" }} />}>
                {campaign?.description && <p style={{ color: "var(--text-3)", fontSize: 14, lineHeight: 1.7, marginBottom: 16 }}>{campaign.description}</p>}
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
                  {[
                    ...
                    <div key={label} style={{ padding: "12px 14px", background: "rgba(255,255,255,0.02)", borderRadius: 10, border: "1px solid rgba(255,255,255,0.06)" }}>
                      <p style={{ fontSize: 11, color: "var(--text-2)", margin: "0 0 4px" }}>{label}</p>
                      <p style={{ fontSize: 13, color: "var(--text-2)", fontWeight: 500, margin: 0 }}>{val}</p>
                    </div>
                  ))}
                </div>
              </Section>
              ...
              {/* 2. Participation */}
              <Section title="2. Participation" icon={<Users size={16} style={{ color: "var(--primary)" }} />}>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, marginBottom: 16 }}>
                  {[
                    ...
                    <div key={label} style={{ textAlign: "center", padding: "16px 10px", background: "rgba(255,255,255,0.02)", borderRadius: 12, border: "1px solid rgba(255,255,255,0.06)" }}>
                      ...
                      <p style={{ color: "var(--text-2)", fontSize: 12, margin: 0 }}>{label}</p>
                    </div>
                  ))}
                </div>
                {campaign?.targetPopulation && (
                  <p style={{ fontSize: 13, color: "var(--text-3)" }}>
                    Population cible : <strong style={{ color: "var(--text-1)" }}>{campaign.targetPopulation.toLocaleString("fr-FR")}</strong> bénéficiaires.
              ...
              <Section title="3. Résultats IQRH collectifs" icon={<TrendingUp size={16} style={{ color: "#34d399" }} />}>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
                  <div>
                    <p style={{ fontSize: 13, color: "var(--text-2)", marginBottom: 6 }}>Score global collectif</p>
                    <p style={{ fontSize: 52, fontWeight: 800, color: "var(--text-1)", margin: 0, lineHeight: 1 }}>
                    ...
                    <div style={{ marginTop: 20, display: "flex", flexDirection: "column", gap: 8 }}>
                      {[
                        ...
                        { label: "Vie Affective", val: stats.averages.affective, color: "var(--primary)" },
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12 }}>
                    {[
                      ...
                      <div key={label} style={{ textAlign: "center", padding: "16px 10px", background: "rgba(255,255,255,0.02)", borderRadius: 12, border: `1px solid ${color}33` }}>
                        ...
                        <p style={{ fontSize: 11, color: "var(--text-2)", margin: 0 }}>{desc}</p>
                      </div>
                    ))}
                  </div>
                </Section>
              ...
              {stats.dominantNeeds && stats.dominantNeeds.length > 0 && (
                <Section title="6. Besoins prioritaires de la population" icon={<TrendingUp size={16} style={{ color: "var(--primary)" }} />}>
                  <p style={{ fontSize: 13, color: "var(--text-2)", lineHeight: 1.6, marginBottom: 16 }}>
                  ...
                  {stats.dominantNeeds.map((n, i) => (
                    <div key={i} style={{ marginBottom: 12 }}>
                      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6, fontSize: 13 }}>
                        <span style={{ color: "var(--text-2)", fontWeight: 500 }}>{n.need}</span>
                        <span style={{ color: "var(--primary)", fontWeight: 600 }}>{n.pct}% des répondants</span>
                      </div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                  {[
                    ...
                    <div key={i} style={{ display: "flex", gap: 14, padding: "14px 16px", background: "rgba(6,182,212,0.04)", borderRadius: 12, border: "1px solid rgba(6,182,212,0.1)" }}>
                      ...
                      <div>
                        <p style={{ fontSize: 14, fontWeight: 600, color: "var(--text-1)", margin: "0 0 4px" }}>{rec.titre}</p>
                        <p style={{ fontSize: 13, color: "var(--text-3)", margin: 0, lineHeight: 1.5 }}>{rec.action}</p>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div style={{ display: "flex", justifyContent: "center", marginTop: 20 }}>
                <button onClick={() => router.push("/dashboard/actions")} className="btn btn-action btn-lg">
