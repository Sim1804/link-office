    ...
    <>
      ...
      <main className="page-main">
        ...
        <div style={{ maxWidth: 760, margin: "0 auto", position: "relative", zIndex: 1 }}>
          ...
          <Stepper currentStep={step} steps={[{ label: "Choix de l'offre" }, { label: "Paramètres" }, { label: "Variables complémentaires" }]} />

          {/* STEP 1 — Choix offre */}
          {step === 1 && (
            <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
              <p style={{ color: "var(--text-3)", fontSize: 14, lineHeight: 1.7, background: "rgba(0,169,157,0.07)", border: "1px solid rgba(0,169,157,0.15)", borderRadius: 12, padding: "14px 18px" }}>
                <Info size={14} style={{ display: "inline", marginRight: 6, color: "var(--primary)" }} />
              ...
              {[
                { id: "PREMIUM", icon: Zap, color: "var(--primary)", border: "var(--primary)", bg: "rgba(0,169,157,0.12)",
                ...
                { id: "PREMIUM_PLUS", icon: Crown, color: "var(--amber)", border: "#f59e0b", bg: "rgba(245,158,11,0.12)",
                  ...
                  <OfferCard
                    ...
                    onSelect={(id) => setForm(f => ({ ...f, offer: id as any }))}
                  />
                ))}
          ...
          {step === 2 && (
            <div className="card">
              <h2 style={{ fontSize: 17, fontWeight: 700, color: "var(--text-1)", marginBottom: 24 }}>Paramètres de la campagne</h2>
              <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
                ...
                <div>
                  <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: "var(--text-3)", marginBottom: 6 }}>Description (optionnelle)</label>
                ...
                <div>
                  <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: "var(--text-3)", marginBottom: 6 }}>Population cible (nombre de bénéficiaires)</label>
                ...
                <div>
                  <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: "var(--text-3)", marginBottom: 6 }}>Logo spécifique de la campagne (Optionnel)</label>
                  ...
                  <p style={{ fontSize: 11, color: "var(--text-3)", marginTop: 6 }}>S'il n'y a pas de logo, le logo par défaut de l'organisation sera utilisé.</p>
                  {uploadError && (
                    <p style={{ fontSize: 12, color: "#f87171", marginTop: 4 }}>⚠ {uploadError}</p>
                  )}
                </div>
              </div>
              {error && <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 16, color: "var(--rose)", fontSize: 13 }}><AlertCircle size={14} />{error}</div>}
              <div style={{ display: "flex", gap: 10, marginTop: 24 }}>
                <button onClick={() => setStep(1)} className="btn btn-tertiary btn-md"><ArrowLeft size={14} /> Retour</button>
                <button
                  onClick={() => {
          ...
          {step === 3 && (
            <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
              <div className="card">
                <h2 style={{ fontSize: 17, fontWeight: 700, color: "var(--text-1)", marginBottom: 8 }}>Variables complémentaires</h2>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  {CAMPAIGN_VARIABLES_LIBRARY.map(v => (
                    <label key={v.id} style={{ display: "flex", alignItems: "flex-start", gap: 12, cursor: "pointer", padding: "12px 14px", borderRadius: 12, border: `1px solid ${selectedVars.includes(v.id) ? "rgba(0,169,157,0.4)" : "var(--surface-2)"}`, background: selectedVars.includes(v.id) ? "rgba(0,169,157,0.08)" : "transparent", transition: "all 0.2s" }}>
                      <input type="checkbox" checked={selectedVars.includes(v.id)} onChange={() => toggleVar(v.id)} style={{ marginTop: 2, accentColor: "var(--primary)" }} />
                      <div>
                        <p style={{ fontSize: 13, fontWeight: 600, color: "var(--text-1)", margin: 0 }}>{v.question}</p>
                        {v.options.length > 0 && <p style={{ fontSize: 11, color: "var(--text-3)", marginTop: 3 }}>Options: {v.options.slice(0,3).join(", ")}{v.options.length > 3 ? `... (+${v.options.length-3})` : ""}</p>}
                      </div>
                    </label>
                  ))}
                </div>
                <div style={{ marginTop: 16 }}>
                  <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: "var(--text-3)", marginBottom: 6 }}>Question personnalisée (optionnelle)</label>
              ...
              {error && <div style={{ display: "flex", alignItems: "center", gap: 8, color: "var(--rose)", fontSize: 13 }}><AlertCircle size={14} />{error}</div>}
              <div style={{ display: "flex", gap: 10 }}>
                <button onClick={() => setStep(2)} className="btn btn-tertiary btn-md"><ArrowLeft size={14} /> Retour</button>
