...
export async function POST(request: Request) {
  ...
  try {
    const { prenom, nom, email: rawEmail, password, codeAccess } = schema.parse(await request.json());
    ...
    const user = await prisma.user.create({ data: { firstName: prenom.trim(), lastName: nom.trim(), email, password: await bcrypt.hash(password, 12), organizationId, campaignId, ...(subscription ? { subscription } : {}), role }, select: { id: true, email: true, firstName: true, lastName: true, role: true, organizationId: true, mustChangePassword: true } });
