...
export async function GET() {
  try {
    ...
    const assessment = await prisma.assessment.findFirst({
      where: { userId: session.user.id, demographic: { isNot: null } },
      include: { demographic: true },
      orderBy: { startedAt: "desc" }
    });

    if (!assessment || !assessment.demographic) {
      return NextResponse.json({ demographic: null });
    }

    return NextResponse.json({ demographic: assessment.demographic });
  } catch (error) {
