    ...
    <div style={{ position: "relative", display: "flex", alignItems: "center" }} ref={dropdownRef}>
      <button 
        ...
        style={{
          ...
          borderRadius: 12, padding: "8px", color: "var(--text-2)", cursor: "pointer",
      ...
      {isOpen && (
        <div style={{
          ...
          border: "1px solid var(--border)", borderRadius: 20, boxShadow: "0 20px 40px rgba(0,0,0,0.6)",
          zIndex: 100, overflow: "hidden"
        }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 20px", borderBottom: "1px solid rgba(18,61,70,0.05)" }}>
            <h3 style={{ fontSize: 14, fontWeight: 700, color: "var(--text-1)" }}>Notifications</h3>
            {unreadCount > 0 && (
              <button style={{ fontSize: 11, color: "var(--primary)", background: "none", border: "none", cursor: "pointer", fontWeight: 600 }}>
            ...
            {notifications.length === 0 ? (
              ...
              notifications.map(notif => (
                <div key={notif.id} style={{ 
                  ...
                  background: notif.status === "UNREAD" ? "rgba(0,169,157,0.05)" : "transparent",
                  ...
                  <div>
                    <h4 style={{ fontSize: 13, fontWeight: 600, color: notif.status === "UNREAD" ? "var(--text-1)" : "var(--text-3)", marginBottom: 4 }}>{notif.title}</h4>
                    <p style={{ fontSize: 12, color: "var(--text-2)", lineHeight: 1.4 }}>{notif.message}</p>
                    {notif.actionLink && (
